<template>
  <div class="root">
    <b-modal ref="addBookModal" id="list-modal" title="Add List" hide-footer>
      ...
      <form action="">
        ...
        <div>
          <button @click.prevent="addList" style="font-size: 28px">
            Submit
          </button>
        </div>
      </form>
...
export default {
  ...
  methods: {
    async addList() {
      if (
        this.formData.list_name != "" &&
        this.formData.list_name.length < 21
      ) {
        try {
          const res = await fetch("http://127.0.0.1:5000/api/user/add_list", {
            method: "post",
            headers: {
        ...
        alert("List name should contain 1-20 character.");
      }
    },
  },
};
